export async function GET(request) {
  ...
  try {
    const url = (type == 'daily')
      ? `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=temperature_2m_max,temperature_2m_min,precipitation_probability_max,weather_code&timezone=Australia%2FSydney`
      : (type == 'hourly')  
      ? `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m,weather_code,precipitation_probability&timezone=auto&start_date=${date}&end_date=${date}`
      : null ;
